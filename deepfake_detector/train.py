                ...
                if phase == "train":
                    ...
                    for imgs, labels in tqdm(train_loader):
                        ...
                        with torch.set_grad_enabled(phase == "train"):
                            ...
                            loss = loss_func(predictions,one_hot.float())
                            sig,thresh_preds = torch.max(predictions,dim=1)
                            """
                            """
                            predictions = model(imgs)
                            # predictions for acc calculation; classification thresh 0.5
                            #thresh_preds : 
                            pred,thresh_preds = torch.max(predictions,dim=1)
                            # predictions for acc calculation; classification thresh 0.5
                            # 32
                            sig = torch.sigmoid(pred)
                            one_hot = torch.nn.functional.one_hot(labels)
                            loss = loss_func(
                                predictions, one_hot.float())
                            """
                            """
                            predictions = model(imgs)
                            sig = torch.sigmoid(predictions)
